...
class TestStudySearch(DatabaseTest):
    ...
    def test_text_query(self):
        ...
        self._assertEqualPublicIds(search.get_results(), [s3, s1])

        search = StudySearch(self.db_session, user=admin, query="BAR")
        self._assertEqualPublicIds(search.get_results(), [s4, s3, s2])

        search = StudySearch(self.db_session, user=admin, query="foobar")
        self._assertEqualPublicIds(search.get_results(), [s3])
    ...
    def test_public_id_query(self):
        ...
        self._assertEqualPublicIds(search.get_results(), [s1])

        search = StudySearch(self.db_session, user=admin, query="smgdb2")
        self._assertEqualPublicIds(search.get_results(), [s2])

        search = StudySearch(self.db_session, user=admin, query="SMGDB30")
        self._assertEqualPublicIds(search.get_results(), [s30])
